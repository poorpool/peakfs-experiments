...
enum GLEAN_IO_MODE { WRITE_CHECKPOINT, READ_RESTART, UNDEF_MODE };
...
enum GLEAN_MPIIO_FILE_PTR { INDEPENDENT_FILE_PTR, SHARED_FILE_PTR };
...
static const int64_t TRANSFER_SIZE = 131072;
...
public:
  ...
  int Write(float *xx, float *yy, float *zz, float *vx, float *vy, float *vz,
            float *phi, int64_t *pid, uint16_t *mask);

  // Return the number of particles for the current rank
  int Read(float *&xx, float *&yy, float *&zz, float *&vx, float *&vy,
           float *&vz, float *&phi, int64_t *&pid, uint16_t *&mask);
...
private:
  ...
  int __POSIX_Write_Data(const char *buf, int64_t *nbytes, off_t start_off);
...
private:
  ...
  int m_totPartitions; // Divide into partitions and write a file out per
                       // partition
