...
RestartIO_GLEAN::RestartIO_GLEAN()
    : m_basePathName(0), m_basePathNameLen(GLEAN_MAX_STRING_LEN), m_dirName(0),
      m_dirNameLen(GLEAN_MAX_STRING_LEN), m_partFileName(0),
      m_partFileNameLen(GLEAN_MAX_STRING_LEN), m_localParticles(-1),
      m_totPartParticles(-1), m_totGlobalParticles(-1), m_mode(UNDEF_MODE),
      m_interface(USE_MPIIO), m_posixAPI(USE_READ_WRITE),
      m_filePtrMode(SHARED_FILE_PTR), m_startTime(0), m_endTime(0),
      m_preallocFile(0), m_fileDist(GLEAN_FILE_PER_RANK) {
...
int RestartIO_GLEAN ::CreateCheckpoint(char *pathname, int64_t &num_particles) {
  ...
  m_dirName = new char[GLEAN_MAX_STRING_LEN];
  memset(m_dirName, '\0', GLEAN_MAX_STRING_LEN * sizeof(char));
  m_dirNameLen = GLEAN_MAX_STRING_LEN;
  snprintf(m_dirName, m_dirNameLen, "%s/dir%04d-%04d", m_basePathName,
           m_partitionID, m_totPartitions);
  ...
  snprintf(m_partFileName, m_partFileNameLen, "%s/test-%04d-%04d.data",
           m_dirName, m_partitionID, m_totPartitions);
  ...
  MPI_Gather(&num_particles, 1, MPI_LONG_LONG, &m_header[HEADER_METAINFO_SIZE],
  ...
  MPI_Gather(&hrank, 1, MPI_LONG_LONG,
             &m_header[HEADER_METAINFO_SIZE + m_partitionSize], 1,
...
int RestartIO_GLEAN ::__POSIX_Create(void) {
  ...
  if (0 == m_partitionRank) {
    fprintf(stdout, "poorpool debug mkdir %s\n", (char *)m_dirName);
    retval = ::mkdir((char *)m_dirName, S_IRWXU);
    fprintf(stdout, "poorpool debug create %s\n", (char *)m_partFileName);
...
int RestartIO_GLEAN ::__MPIIO_Create(void) {
  ...
  retval = MPI_File_open(m_fileComm, (char *)m_partFileName, MPI_MODE_WRONLY,
...
int64_t RestartIO_GLEAN ::OpenRestart(char *pathname) {
  ...
  MPI_Scatter(&m_header[HEADER_METAINFO_SIZE], 1, MPI_LONG_LONG,
              &m_localParticles, 1, MPI_LONG_LONG, 0, m_partitionComm);

  int64_t rank_in_file, g_rank;

  // Scatter the global ranks
  MPI_Scatter(&m_header[HEADER_METAINFO_SIZE + m_partitionSize], 1,
              MPI_LONG_LONG, &rank_in_file, 1, MPI_LONG_LONG, 0,
              m_partitionComm);
...
int RestartIO_GLEAN ::__MPIIO_Open_Restart(void) {
  ...
  status = MPI_File_open(m_fileComm, (char *)m_partFileName, MPI_MODE_RDONLY,
                         MPI_INFO_NULL, &m_fileHandle);
...
int RestartIO_GLEAN ::__MPIIO_Read_Header(void) {
  ...
  if (0 == m_partitionRank) {
    ...
    errcode = MPI_File_open(MPI_COMM_SELF, (char *)m_partFileName,
                            MPI_MODE_RDONLY, MPI_INFO_NULL, &m_fileHandle);
    assert(errcode == MPI_SUCCESS);

    ofst = 0;
    errcode = MPI_File_read_at(m_fileHandle, ofst, m_header, m_headerSize,
                               MPI_BYTE, &status);
...
int RestartIO_GLEAN ::__POSIX_Write_Data(const char *buf, int64_t *nbytes,
                                         off_t start_off) {
  ...
  while (nleft > 0) {
    int64_t transfer_size = min(nleft, TRANSFER_SIZE);
    ...
    if (m_posixAPI == USE_READ_WRITE) {
      ...
      nwrite = write(m_posixFD, buf, transfer_size);
    } else {
      nwrite = pwrite64(m_posixFD, buf, transfer_size, start_off);
    }
...
int64_t RestartIO_GLEAN ::__POSIX_Read_Data(unsigned char *buf, int64_t &len,
                                            off_t start_off) {
  ...
  while (nleft > 0) {
    ...
    if (m_posixAPI == USE_READ_WRITE) {
      ...
      nread = ::read(m_posixFD, buf, transfer_size);
    } else {

      nread = ::pread64(m_posixFD, buf, transfer_size, offset);
    }
...
int RestartIO_GLEAN ::Close(void) {
  ...
  MPI_Reduce(&part_size, &tot_data_size, 1, MPI_LONG_LONG, MPI_SUM, 0,
             m_globalComm);
  ...
  if (0 == m_globalCommRank) {
    ...
    switch (m_mode) {
    case READ_RESTART:
      cout << " READ Restart Perf: " << bw << " BW[MiB/s] " << tot_data_size
           << " Bytes " << agg_max_time << " MaxTime[sec] " << endl;
      break;
    case WRITE_CHECKPOINT:
      cout << " WRITE Checkpoint Perf: " << bw << " BW[MiB/s] " << tot_data_size
           << " Bytes " << agg_max_time << " MaxTime[sec] " << endl;
      break;
    }
  }
  ...
  MPI_Reduce (&tot_time, &part_max_time, 1, MPI_DOUBLE, MPI_MAX, 0,
  m_partitionComm); MPI_Reduce (&tot_time, &part_min_time, 1, MPI_DOUBLE,
  MPI_MIN, 0, m_partitionComm); if (0 == m_partitionRank)
  {
      cout << "-------- Partition Performance: ID  " << m_partitionID << " Of "
  << m_totPartitions   << endl; double bw = ((double)m_partFileSize)/
  part_max_time / (1024.0 * 1024.0);

      switch (m_mode)
      {
          case READ_RESTART:
              cout << " READ Restart Perf: " << bw << " BW[MB/s] " <<
  m_partFileSize << " Bytes " << part_max_time << " MaxTime[sec] " <<  endl;
              break;
          case WRITE_CHECKPOINT:
              cout << " WRITE Checkpoint Perf: " << bw <<" BW[MB/s] " <<
  m_partFileSize << " Bytes " << part_max_time << " MaxTime[sec] " <<  endl;
              break;
      }
...
void RestartIO_GLEAN::PrintIOCoordInfo(void) {
  ...
  cout << "G: " << m_globalCommRank << " " << m_globalCommSize
       << " P: " << m_totPartitions << " " << m_partitionID << " "
       << m_idInPartition << " " << m_partitionRank << " " << m_partitionSize
       << endl;
...
int RestartIO_GLEAN::__initalize_BGQ_PartitionInfo(void) {
  ...
  color = (A_color * (B_blocks * C_blocks * D_blocks * E_blocks)) +
          ...
          (C_color * (D_blocks * E_blocks)) + (D_color * (E_blocks)) + E_color;
  ...
  ranks_per_node = m_globalCommSize / (Anodes * Bnodes * Cnodes * Dnodes *
  Enodes);
  ...
  m_idInPartition = (A_id * (SPLIT_B * SPLIT_C * SPLIT_D * SPLIT_E *
  ranks_per_node))
...
int RestartIO_GLEAN ::__MPIIO_Close_Checkpoint(void) {
  ...
  if (0 == m_partitionRank) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, m_header, m_headerSize,
                                MPI_BYTE, &status);
    if (MPI_SUCCESS != errcode) {
...
//
//----------------------------------------------------------------------------
int RestartIO_GLEAN ::Write(float *xx, float *yy, float *zz, float *vx,
                            float *vy, float *vz, float *phi, int64_t *pid,
                            uint16_t *mask) {
  MPI_Status status;
  int errcode, retval;
  ...
  MPI_Exscan(&num_particles, &scan_size, 1, MPI_LONG_LONG, MPI_SUM,
             m_partitionComm);
  ...
  if (m_interface == USE_POSIX) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, xx, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, yy, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, zz, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, vx, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, vy, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, vz, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, phi, num_particles,
                                MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, pid, num_particles,
    ...
    errcode = MPI_File_write_at(m_fileHandle, ofst, mask, num_particles,
                                MPI_UNSIGNED_SHORT, &status);
    if (MPI_SUCCESS != errcode) {
...
//
//----------------------------------------------------------------------------
int RestartIO_GLEAN::Read(float *&xx, float *&yy, float *&zz, float *&vx,
                          float *&vy, float *&vz, float *&phi, int64_t *&pid,
                          uint16_t *&mask)

{
  ...
  int64_t record_size =
      (7 * sizeof(float)) + sizeof(int64_t) + sizeof(uint16_t);
  ...
  if (m_interface == USE_MPIIO) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, xx, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, yy, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, zz, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, vx, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, vy, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, vz, num_particles, MPI_FLOAT,
                               &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, phi, num_particles,
                               MPI_FLOAT, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, pid, num_particles,
                               MPI_LONG_LONG, &status);
    if (MPI_SUCCESS != errcode) {
    ...
    errcode = MPI_File_read_at(m_fileHandle, ofst, mask, num_particles,
                               MPI_UNSIGNED_SHORT, &status);
    if (MPI_SUCCESS != errcode) {
